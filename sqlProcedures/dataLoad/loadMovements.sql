...
SELECT 
    dato.value('@ValorDocId', 'INT'),
    dato.value('@IdTipoMovimiento', 'INT'),
    dato.value('@Fecha', 'DATE'),
    dato.value('@Monto', 'DECIMAL(10,2)'),
    CASE 
        WHEN TM.TipoAccion = 'Credito' THEN E.SaldoVacaciones + dato.value('@Monto', 'DECIMAL(10,2)')
        WHEN TM.TipoAccion = 'Debito' THEN E.SaldoVacaciones - dato.value('@Monto', 'DECIMAL(10,2)')
        ELSE E.SaldoVacaciones -- Por si acaso no coincide ningún tipo
    END AS NuevoSaldo,
    U.id,
    dato.value('@PostInIP', 'VARCHAR(64)'),
    dato.value('@PostTime', 'DATETIME')
FROM @xmlData.nodes('/Datos/Movimientos/movimiento') AS MY_XML(dato)
JOIN dbo.Usuario U ON U.Nombre = dato.value('@PostByUser', 'VARCHAR(64)')
JOIN dbo.Empleado E ON E.ValorDocumentoIdentidad = dato.value('@ValorDocId', 'INT')
JOIN dbo.TipoMovimiento TM ON TM.id = dato.value('@IdTipoMovimiento', 'INT');